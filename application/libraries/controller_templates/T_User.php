...
class T_User extends MY_Controller { 
    ...
    public function view($view_file, $rID, $ContactId=NULL){        
        ...
        if( $this->session->userdata('_AdminLevel') >= ADMIN_LEVEL_SUPERVISOR ) $this->data['view_setup']['show_user_table'] = TRUE;
    ...
    public function view_modal($view_file, $rID) {
    ...
    public function change_password($view_file, $rID) {  
        ...
        //refresh page
        redirect(DATAOWNER_ID . '/' . $this->controller_name . '/view/' . $view_file . '/' . $rID . '/' . $rID);
