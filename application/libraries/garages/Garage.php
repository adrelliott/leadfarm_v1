            ...
            foreach ($data as $key => $value)
            {
                switch ($key)
                {
                    case '__MOT_expiry':
                         ...
                         {
                             $n[$data['__Id']][] =  '<span class="notification warning">The MOT is expired, or about to expire, for <a href="' . site_url() . DATAOWNER_ID . '/vehicles/view/edit/' . $data['__Id'] . '/' . $ContactId . '" class="">' . $identifier . '</a>.(Expiry date recorded is <strong>' . $date . '</strong>)</span>';
                         }
                         break;
                    case '__Service_expiry':
                         ...
                         if ($timestamp - time() <= $countdown)
                         {
                            $n[$data['__Id']][] =  '<span class="notification warning">The SERVICE is expired, or about to expire, for <a href="' . site_url() . DATAOWNER_ID . '/vehicles/view/edit/' . $data['__Id'] . '/' . $ContactId . '" class="">' . $identifier . '</a>. (Expiry date recorded is <strong>' . $date . '</strong>)</span>';
                         }
                         break;
                }
            }
